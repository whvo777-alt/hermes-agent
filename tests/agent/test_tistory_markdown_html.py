...
def test_toc_section_becomes_blue_gray_box():
    ...
    assert 'padding:18px 24px;margin:28px 0;border-radius:6px;">' in html
...
def test_toc_h2_does_not_consume_automatic_section_number():
    ...
    assert ">1. </span>" not in h2_blocks[0]
    assert ">1. </span>" in h2_blocks[1]
    assert ">2. </span>" in h2_blocks[2]


def test_toc_box_contains_all_items_until_next_h2():
    markdown = "## 목차\n\n- 항목 하나\n- 항목 둘\n- 항목 셋\n\n## 첫 섹션\n본문"
    html = markdown_html.markdown_to_tistory_html(markdown)
    toc_box = re.search(
...
def test_core_whole_paragraph_becomes_blue_callout():
    ...
    assert '<span style="color:#1b4f80;font-weight:bold;">핵심:</span>' in html
    assert "반드시 확인하세요." in html
...
def test_caution_whole_paragraph_becomes_orange_callout():
    ...
    assert '<span style="color:#a86a10;font-weight:bold;">주의:</span>' in html
    assert "무리하지 마세요." in html
...
def test_disclaimer_whole_paragraph_becomes_dashed_gray_callout():
    ...
    assert "면책:" in html and "일반적인 정보입니다." in html


def test_inline_strong_text_is_red():
    html = markdown_html.markdown_to_tistory_html("이것은 **굵은 글씨**입니다.")

    assert '<strong style="color:#c0392b;">굵은 글씨</strong>' in html
