...
_FLOW_CHUNKS = {
    "문제 해결형": "확인할 것들을 목록으로 정리한 대목",
    "정보 설명형": "짧은 문답 두 개 (질문 한 줄, 답변 한두 문장)",
    "비교형": "가로 줄 3개에서 5개짜리 표 하나 (첫 줄은 칸 이름)",
    "단계별 가이드형": "번호를 붙인 절차 4개에서 6개",
    "상황별형": "상황과 확인할 점을 나란히 놓은 표 하나",
}
...
def _expression_goals(*, platform_id: str, category_id: str, flow_seed: str = "") -> str:
    ...
    if platform_id in ("wordpress", "blogspot"):
        ...
        h2_rule = (
            ...
            "다른 흐름을 고르지 않는다.\n"
            f"- 본문 안에 {_FLOW_CHUNKS[flow]} 을(를) 반드시 하나 넣는다. "
            "어느 대단원에 넣을지는 내용에 맞게 정한다."
        )
    else:
...
- 【강조는 불규칙하게 — 단조로운 결론투 금지】
  - 강조는 두 갈래로 나눠 쓴다. 주의할 것과 하지 말아야 할 것을 5곳에서 7곳, 핵심과 꼭 기억할 것을 5곳에서 7곳.
  - 두 갈래 모두 별표 두 개로 감싼다. 문단마다 넣지 말고 글 전체에 고르게 흩는다.
...
- 제목에 숫자를 기본으로 넣지 않는다. 본문에서 개수가 분명하게 떨어지는 주제일 때만 예외로 쓴다.
- 포커스 키워드는 본문 6~12회만 자연스럽게.
...
- 독자가 “무엇을 확인·피하면 되는지”가 남도록 쓴다. 치료·효과 보장 금지."""
    ...
    if category_id == "self-dev" or platform_id == "blogspot":
        faq_rule = (
            "- 글의 주제에 맞는 설명과 다음 생각거리를 남긴다. "
            "자주 묻는 질문 절은 플랫폼 규칙을 따른다. "
            "각 질문은 `질문: …` 다음 줄에 `답변: …` 형식(각 답변 2~4문장)."
            if platform_id in ("wordpress", "blogspot")
            else "- 글의 주제에 맞는 설명과 다음 생각거리를 남긴다. FAQ는 생략하거나 Q 2개만."
        )
...
def write_blog_post(*, platform_id: str, platform_label: str, category_id: str, category_name: str,
    ...
    if source_links:
        ...
        source_block = (
            ...
            "- 설명한 문단 바로 다음 줄에 링크만 홀로 놓는다. 문장 안에 섞지 않는다.\n"
            "- 글 끝에 몰아넣지 않는다.\n"
            "- 같은 주소를 두 번 넣지 않는다.\n"
...
{length_h2_line}
- 건강 주제면 전문가 상담 권장, 자기계발 주제면 주제에 따라 개념 설명·비교·사례·절차 중 맞는 방식을 고른다.
- AI 요약체·양산 블로그 재탕·가짜 후기로 분량을 채우지 않는다. 대비 표현은 매번 다른 방식(질문형/일화형/서술형)으로.
- 볼드는 아껴 쓴다(모든 단락 끝마다 X). 구조(단계/표/체크리스트 유무·개수)는 매번 다르게 가져간다."""
