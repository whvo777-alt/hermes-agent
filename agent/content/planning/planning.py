...
- 계획에는 검색 의도 기반 제목 방향, 핵심 키워드, 독자 문제, 글 구조, 이미지 계획, 모바일 가독성, 신뢰 보강 포인트를 포함한다.
- 내부 topic_title이나 고정 문장 틀을 제목 방향으로 그대로 복사하지 않는다.
- 【절대 규칙】 "글 구조" 항목의 맨 첫 줄에 반드시 아래 한 줄 형식으로 구조 배정을 명시한다
  (전부가 아니라 H2 섹션 중 2~3개에만 배정, 나머지는 자유 서술 — 매 섹션에 강제하지 않는다):
  `- 구조 배정: N번=체크리스트, M번=표 비교 (또는 단계형 등)`
  이 줄은 다른 항목보다 먼저, 짧고 압축된 한 줄로 써야 한다 — 뒤에 나오는 상세 설명이 길어지면
  요약 단계에서 이 줄이 잘려나가 작성 단계에 전달되지 않을 수 있다.
...
포함할 내용:
1. 블로그 1편 기획
2. 플랫폼별 글 구조 (섹션 중 2~3개는 체크리스트/표 비교/단계 등 구체 구조로, 나머지는 자유 서술로 배정 — 이 항목의 첫 줄에 압축된 "구조 배정" 한 줄을 먼저 쓴다)
3. 대표 이미지/본문 이미지 계획
4. 품질검수 체크포인트
5. 민감 표현/플랫폼 주의사항
